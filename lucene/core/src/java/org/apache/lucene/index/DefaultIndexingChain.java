...
final class DefaultIndexingChain extends DocConsumer {
  ...
  private PerField[] fields = new PerField[1];  // document的每个field 执行invert 后的信息
  ...
  public Sorter.DocMap flush(SegmentWriteState state) throws IOException {
    ...
    writeNorms(state, sortMap);   // file : .nvm ,  .nvd
    ...
    storedFieldsConsumer.flush(state, sortMap);  // file .fdt .fdx
    if (docState.infoStream.isEnabled("IW")) {
